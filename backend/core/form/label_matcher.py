...
class LabelMatcher:
    ...
    Convention retenue (courante sur ce type de formulaire) :
    1. le libellé est sur la MÊME LIGNE, immédiatement à GAUCHE du champ
       (ex: "Délivré le: ______") -> priorité la plus haute ;
    2. sinon, le libellé est juste AU-DESSUS du champ (ex: au-dessus d'une
       zone de texte ou d'une case à cocher isolée) ;
    3. sinon, le bloc de texte le plus proche, mais seulement s'il reste
       dans un rayon raisonnable (sinon on laisse le libellé vide plutôt
       que d'assigner un texte sans rapport).
    """
    ...
    def _find_nearest_within_radius(self, field: FormField, text_blocks: list[TextBlock]):
        ...
        for block in text_blocks:
            ...
            bbox = block.bbox
            distance = (
                (rect.center_x - bbox.center_x) ** 2
                + (rect.center_y - bbox.center_y) ** 2
            ) ** 0.5
